...
{
    [TLVTag(208)]
    public class CreadiTBalnceSmart : ModelBase
    {
        ...
        public double OverdraftCredit { get; set; }

        public double ConsumedCredit { get; set; }

        public double CumulativeCharges { get; set; }

        public DateModel AppDate { get; set; }

        public override byte[] Serialize()
        {
            return SerializeAttributes( RemainCredit, OverdraftCredit, ConsumedCredit, CumulativeCharges).Concat(AppDate.Serialize()).ToArray();
        }

        public static CreadiTBalnceSmart Deserialize(byte[] data)
        {
            if (data != null && data.Length != 0)
            {
                return new CreadiTBalnceSmart
                {
                  
                    RemainCredit = BitConverter.ToDouble(data, 0),
                    OverdraftCredit = BitConverter.ToDouble(data, 8),
                    ConsumedCredit = BitConverter.ToDouble(data, 16),
                    CumulativeCharges = BitConverter.ToDouble(data, 24),
                    AppDate = DateModel.Deserialize(data.Skip(32).Take(4).ToArray())
                };
            }
